...
class Helper {
	...
	public static function get_posts( $args = [], $show_heading = false, $show_cached = false ) {
		...
		// use cache
		if( true === $show_cached && ( $cached_posts = wp_cache_get( "wpd_{$_args['post_type']}", 'wpd' ) ) ) {
			$posts = $cached_posts;
		}
		...
		// don't use cache
			...
			wp_cache_add( "wpd_{$_args['post_type']}", $posts, 'wpd', 3600 );
		}

		$posts = $show_heading ? [ '' => sprintf( __( '- Choose a %s -', 'wpd' ), $_args['post_type'] ) ] + $posts : $posts;

		return apply_filters( 'wpd_get_posts', $posts, $_args );
	}
	 ...
	 *
	 * It'll look into /wp-did directory of your active theme
	 * first. if not found, default template will be used.
	 * can be overwriten with wp-did_template_overwrite_dir hook
	 *
	...
	public static function get_template( $slug, $base = 'views', $args = null ) {
		...
		$overwrite_template_dir = apply_filters( 'wpd_template_overwrite_dir', get_stylesheet_directory() . '/wp-did/', $slug, $base, $args );
		
		// default template directory
		$plugin_template_dir = dirname( WP_DID ) . "/{$base}/";
		...
		elseif ( file_exists( $plugin_template_path ) ) {
			...
			return __( 'Template not found!', 'wp-did' );
		}
	}
}