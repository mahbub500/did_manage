...
final class Plugin {
	...
	private function define() {
		 ...
		 * @since 0.9
		 */
		define( 'WP_DID', __FILE__ );
		define( 'WP_DID_DIR', dirname( WP_DID ) );
		define( 'WP_DID_ASSET', plugins_url( 'assets', WP_DID ) );
		define( 'WP_DID_DEBUG', apply_filters( 'wp-did_debug', true ) );
		 ...
		 * @var $plugin
		 */
		$this->plugin					= get_plugin_data( WP_DID );
		$this->plugin['basename']		= plugin_basename( WP_DID );
		$this->plugin['file']			= WP_DID;
		$this->plugin['server']			= apply_filters( 'wp-did_server', 'https://codexpert.io/dashboard' );
